...
class MarketDataProvider:
    ...
    Pass a MT5Bridge instance via the ``bridge`` parameter so that the
    MT5_TERMINAL source can fetch real price and OHLCV data.  When no
    bridge is provided the MT5 source is silently skipped and the next
    fallback is tried.
    """

    def __init__(self, primary_source: DataSource = DataSource.MT5_TERMINAL, bridge=None):
        self.primary_source = primary_source
        self.bridge = bridge            # Optional MT5Bridge
    ...
    def _fetch_tick_mt5(self, symbol: str) -> Optional[Tick]:
        """Fetch current bid/ask from MT5 via the file bridge."""
        if self.bridge is None:
            logger.debug("MT5 bridge not provided — skipping MT5 tick fetch")
            return None
        result = self.bridge.get_price(symbol)
        if result is None:
            return None
        bid  = float(result.get('bid', 0.0))
        ask  = float(result.get('ask', 0.0))
        last = float(result.get('last', (bid + ask) / 2))
    ...
    def _fetch_ohlcv_mt5(self, symbol: str, timeframe: Timeframe, bars: int) -> List[OHLCV]:
        """Fetch OHLCV bars from MT5 via the file bridge."""
        if self.bridge is None:
            logger.debug("MT5 bridge not provided — skipping MT5 OHLCV fetch")
            return []
        period_map = {
            Timeframe.M1:  1,
            Timeframe.M5:  5,
        ...
        raw_bars = self.bridge.get_ohlcv(symbol, period, bars)
        if not raw_bars:
            return []
        result = []
        for b in raw_bars:
            try:
                ...
                logger.warning(f"Skipping bad OHLCV bar: {e}")
        logger.info(f"MT5 OHLCV: received {len(result)} bars for {symbol} {timeframe.value}")
        return result
